...
def select_dir():
    ...
    root.title("File Organizer [himanshudhi-004] | Status : Organizing")
...
def mapping(dir, list_dir, ext_types, loading_bar):
    ...
    loading_bar.destroy()
    os.startfile(dir)
    root.title("File Organizer [himanshudhi-004] | Status : Idle")


def github():
    # Opening Github or contact
    url = "https://github.com/himanshudhi-004"
...
def edit_settings():
    ...
    root.title("File Organizer [himanshudhi-004] | Status : Settings")
    app = CTk()
    app.title("Settings")
    ...
    def save_file(new_settings):
        root.title("File Organizer [himanshudhi-004] | Status : Save")
    ...
    back = CTkButton(back_save, text="Back", font=(
        'Sans Serif', 15), text_color=settings['c2'], command=lambda: [root.title("File Organizer [himanshudhi-004] | Status : Idle"), app.destroy()])
    back.pack(side="bottom", padx=10, pady=10)
    app.protocol("WM_DELETE_WINDOW", lambda: [root.title(
        "File Organizer [himanshudhi-004] | Status : Idle"), app.destroy()])
    app.mainloop()
